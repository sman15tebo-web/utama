...
function getValidImg(url, fallback) {
    if (!url) return fallback;
    var s = url.toString();
    // Jika base64, langsung pakai
    if (s.startsWith('data:image')) return s;
    // Jika URL Cloudinary / URL eksternal lain, langsung pakai
    if (s.startsWith('http') && s.indexOf('drive.google.com') === -1) return s;
    // Jika Drive URL, konversi ke format uc?export=view
    var id = '';
    if (s.indexOf('/file/d/') !== -1) { id = s.split('/file/d/')[1].split('/')[0]; }
    else if (s.indexOf('id=') !== -1) { id = s.split('id=')[1].split('&')[0]; }
