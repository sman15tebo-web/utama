function renderSemuaData(data) {
    try {
        ...
        var tSis = '', tAlm = '', tSisPub = '', tAlmPub = '', cSiswa = 0, cAlumni = 0; var sL = 0, sP = 0, aL = 0, aP = 0;
        for (var x = 0; x < (data.siswa || []).length; x++) {
            var s = data.siswa[x]; if (!s.kategori) continue;
            var kat = (s.kategori || '').toString().trim();
            ...
            var aksiBtn = '<td class="p-3 text-center whitespace-nowrap"><button onclick="siapkanEdit(\'siswa\',\'' + s.id + '\')" class="text-blue-500 bg-blue-100 px-3 py-1 rounded-full mr-1"><i class="fas fa-edit"></i></button><button onclick="hapusData(\'siswa\',\'' + s.id + '\')" class="text-red-500 bg-red-100 px-3 py-1 rounded-full"><i class="fas fa-trash"></i></button></td>';
            var trBase = '<td class="p-4 font-bold">' + amankanTeks(s.label) + pdfIco + '</td><td class="p-4 text-center text-blue-500 font-bold">' + l + '</td><td class="p-4 text-center text-pink-500 font-bold">' + p + '</td><td class="p-4 text-center font-black">' + tot + '</td>';

            if (kat === 'Siswa Aktif') { cSiswa += tot; sL += l; sP += p;
                tSis += '<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">' + trBase + aksiBtn + '</tr>';
                tSisPub += '<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800"><td class="p-4 font-bold text-primary dark:text-white">' + amankanTeks(s.label) + pdfIco + '</td><td class="p-4 text-center text-blue-500 font-bold">' + l + '</td><td class="p-4 text-center text-pink-500 font-bold">' + p + '</td><td class="p-4 text-center font-black">' + tot + '</td></tr>'; }
            else if (kat === 'Alumni') { cAlumni += tot; aL += l; aP += p;
                tAlm += '<tr class="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">' + trBase + aksiBtn + '</tr>';
        ...
        // Render tabel berita & galeri milik pegawai sendiri
        if ((curRole === 'guru' || curRole === 'tu') && typeof renderTabelBeritaPegawai === 'function') {
            renderTabelBeritaPegawai();
            renderTabelGaleriPegawai();
        }
